...
const reactPattern: PatternDef = {
  ...
  nodes: [
    { id: "input",   label: "User Query", x: 8,  y: 50, shape: "circle",  category: "io",       icon: <Inbox className={ic} />,    sampleOutput: "The initial question or task from the user" },
    { id: "thought", label: "Thought",    sub: "Reason",  x: 30, y: 50, category: "reasoning", icon: <Brain className={ic} />,    sampleOutput: "The agent reasons about what action to take next based on current context" },
    { id: "action",  label: "Action",     sub: "Tool",    x: 52, y: 25, category: "tool",      icon: <Wrench className={ic} />,   sampleOutput: "Executes a tool call or API request chosen by the reasoning step" },
    { id: "observe", label: "Observe",    sub: "Result",  x: 68, y: 50, category: "data",      icon: <Eye className={ic} />,      sampleOutput: "Collects and interprets the result returned from the action" },
    { id: "done",    label: "Done?",                      x: 84, y: 50, shape: "diamond", category: "decision", icon: <GitBranch className={ic} />, sampleOutput: "Decides whether the goal is satisfied or another loop is needed" },
    { id: "output",  label: "Output",                     x: 94, y: 25, shape: "circle",  category: "io",       icon: <Send className={ic} />,     sampleOutput: "The final answer delivered to the user after loop completion" },
  ],
  edges: [
...
const toolPattern: PatternDef = {
  ...
  complexity: "Low", risk: "Low",
  nodes: [
    { id: "input",    label: "User Query",      x: 8,  y: 50, shape: "circle", category: "io",       icon: <Inbox className={ic} />,  sampleOutput: "A complex request requiring multiple external tools" },
    { id: "select",   label: "Select Tool(s)",  x: 35, y: 50, category: "tool",      icon: <Brain className={ic} />,  sampleOutput: "Identifies which tools are needed and dispatches them in parallel" },
    { id: "tool1",    label: "Web Search",       x: 56, y: 20, shape: "pill", category: "tool",      icon: <Wrench className={ic} />, sampleOutput: "Searches the web for real-time information" },
    { id: "tool2",    label: "Slack",            x: 56, y: 50, shape: "pill", category: "tool",      icon: <Wrench className={ic} />, sampleOutput: "Sends notifications or retrieves messages from Slack" },
    { id: "tool3",    label: "Vector DB",        x: 56, y: 80, shape: "pill", category: "memory",    icon: <Wrench className={ic} />, sampleOutput: "Retrieves semantically relevant documents from vector storage" },
    { id: "results",  label: "Aggregate",        x: 76, y: 50, category: "data",      icon: <FileText className={ic} />, sampleOutput: "Merges results from all parallel tool calls into unified context" },
    { id: "generate", label: "Generate",         x: 88, y: 50, category: "reasoning", icon: <Brain className={ic} />,  sampleOutput: "Synthesizes aggregated results into a coherent response" },
    { id: "output",   label: "Response",         x: 96, y: 50, shape: "circle", category: "io",       icon: <Send className={ic} />,   sampleOutput: "Final answer combining insights from all tools" },
  ],
  edges: [
...
const planPattern: PatternDef = {
  ...
  nodes: [
    { id: "input",    label: "User Query",  x: 8,  y: 40, shape: "circle",  category: "io",       icon: <Inbox className={ic} />,    sampleOutput: "A multi-step task that requires structured decomposition" },
    { id: "plan",     label: "Make a Plan", x: 28, y: 40, category: "reasoning", icon: <Brain className={ic} />,    sampleOutput: "Decomposes the goal into ordered sub-tasks with dependency tracking" },
    { id: "task1",    label: "Task 1",      x: 42, y: 62, shape: "pill",   category: "data",      icon: <FileText className={ic} />, sampleOutput: "First sub-task derived from the plan" },
    { id: "task2",    label: "Task 2",      x: 42, y: 75, shape: "pill",   category: "data",      icon: <FileText className={ic} />, sampleOutput: "Second sub-task, may depend on Task 1 output" },
    { id: "task3",    label: "Task 3",      x: 42, y: 88, shape: "pill",   category: "data",      icon: <FileText className={ic} />, sampleOutput: "Third sub-task, builds on previous results" },
    { id: "exec",     label: "Execute",     x: 58, y: 40, category: "tool",      icon: <Cpu className={ic} />,      sampleOutput: "Runs each sub-task sequentially, passing context between steps" },
    { id: "tools",    label: "Tools",       x: 60, y: 68, shape: "pill",   category: "tool",      icon: <Wrench className={ic} />,   sampleOutput: "External tools invoked by the executor for each sub-task" },
    { id: "check",    label: "All done?",   x: 76, y: 40, shape: "diamond", category: "decision", icon: <GitBranch className={ic} />, sampleOutput: "Checks if all sub-tasks are complete or re-planning is needed" },
    { id: "generate", label: "Generate",    x: 88, y: 68, category: "reasoning", icon: <Brain className={ic} />,    sampleOutput: "Combines all sub-task outputs into a unified final result" },
    { id: "output",   label: "Response",    x: 8,  y: 80, shape: "circle",  category: "io",       icon: <Send className={ic} />,     sampleOutput: "The completed deliverable assembled from all sub-tasks" },
  ],
  edges: [
    { from: "input",    to: "plan" },
    { from: "plan",     to: "exec" },
...
const reflectPattern: PatternDef = {
  ...
  complexity: "Medium", risk: "Medium",
  nodes: [
    { id: "input",     label: "User Query", x: 8,  y: 35, shape: "circle",  category: "io",       icon: <Inbox className={ic} />,    sampleOutput: "A quality-critical task where the first draft is rarely sufficient" },
    { id: "tools",     label: "Tools",      x: 30, y: 10, shape: "pill",   category: "tool",      icon: <Wrench className={ic} />,   sampleOutput: "Available tools for execution: code runner, linter, test suite, etc." },
    { id: "execute",   label: "Execute",    x: 30, y: 40, category: "tool",      icon: <Cpu className={ic} />,      sampleOutput: "Produces an initial draft using available tools and context" },
    { id: "draft",     label: "Draft",      x: 50, y: 40, shape: "pill",   category: "data",      icon: <FileText className={ic} />, sampleOutput: "The first-pass output before quality evaluation" },
    { id: "reflect",   label: "Reflect",    x: 68, y: 55, category: "reasoning", icon: <Eye className={ic} />,      sampleOutput: "Critically evaluates the draft against quality criteria and goals" },
    { id: "response",  label: "Response",   x: 68, y: 25, category: "data",      icon: <FileText className={ic} />, sampleOutput: "Quality assessment with identified gaps and improvement areas" },
    { id: "improved",  label: "Improved",   x: 84, y: 55, category: "data",      icon: <FileText className={ic} />, sampleOutput: "Specific improvements to apply in the next iteration" },
    { id: "output",    label: "Final",      x: 8,  y: 75, shape: "circle",  category: "io",       icon: <Send className={ic} />,     sampleOutput: "The polished output after one or more reflection cycles" },
  ],
  edges: [
